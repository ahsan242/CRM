...
import { getCalculatedPrice } from '@/helpers/techproduct';
    ...
    row: {
      original: {
        ...
        <div className="flex-shrink-0 me-3">
          <Link to={`/ecommerce/techproducts/${id}`}>
            <img src={images[0]} alt={name} className="img-fluid avatar-sm" />
          </Link>
        </div>
        <div className="flex-grow-1">
          <h5 className="mt-0 mb-1">
            <Link to={`/ecommerce/techproducts/${id}`} className="text-reset">
              {name}
            </Link>
          </h5>
...
const TechProductsListTable = ({
  ...
  return <ReactTable columns={columns} data={techproducts} rowsPerPageList={pageSizeList} pageSize={10} tableClass="text-nowrap mb-0" theadClass="bg-light bg-opacity-50" showPagination />;
};
