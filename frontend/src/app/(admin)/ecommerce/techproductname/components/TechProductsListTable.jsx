...
import { getCalculatedPrice } from '@/helpers/techproductname';
import { getStockStatus } from '@/utils/other';
const columns = [{
  header: 'TechProductName Name',
  cell: ({
    row: {
      original: {
        id,
        ...
        <div className="flex-shrink-0 me-3">
          <Link to={`/ecommerce/techproductnames/${id}`}>
            <img src={images[0]} alt={name} className="img-fluid avatar-sm" />
          </Link>
        </div>
        <div className="flex-grow-1">
          <h5 className="mt-0 mb-1">
            <Link to={`/ecommerce/techproductnames/${id}`} className="text-reset">
              {name}
            </Link>
          </h5>
...
const TechProductNamesListTable = ({
  ...
  return <ReactTable columns={columns} data={techproductnames} rowsPerPageList={pageSizeList} pageSize={10} tableClass="text-nowrap mb-0" theadClass="bg-light bg-opacity-50" showPagination />;
};
