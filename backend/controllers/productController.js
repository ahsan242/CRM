...
// // 📌 IMPROVED Helper function: download image to uploads folder with better error handling
...
//     if (!response.data) {
//       console.log(`❌ No image data received from: ${url}`);
//       return null;
//     }
...
//           } else {
//             console.log(
//               `❌ Image file is too small (${stats.size} bytes), likely invalid`
//             );
...
//         // Try to delete the file if it was partially created
//         if (fs.existsSync(filePath)) {
...
//         }
//         resolve(null);
//       }, 20000); // 20 second timeout for write operation
//     });
//   } catch (error) {
...
// // IMPROVED: Helper function to process gallery images with better error handling
...
//     console.log(
//       `🧹 Cleared existing gallery images for product ID: ${productId}`
//     );

//     let successfulDownloads = 0;
//     const maxRetries = 2; // Maximum number of retry attempts

//     for (const [index, img] of galleryData.entries()) {
//       try {
//         // Try different image URL priorities
//         const imgUrl = img.Pic500x500 || img.Pic || img.LowPic || img.HighPic;
//         if (!imgUrl) {
//           console.log(`⚠️ No image URL found for gallery image ${index + 1}`);
//           continue;
//         }
...
//         // Retry logic with different URL fallbacks
//         while (!downloadedFilename && retryCount <= maxRetries) {
//           try {
//             const timestamp = Date.now();
//             const imageExt = path.extname(imgUrl.split("?")[0]) || ".jpg";
//             const galleryImageFilename = `icecat_${productCode}_gallery_${
//               index + 1
...
//             downloadedFilename = await downloadImage(
//               imgUrl,
//               galleryImageFilename
//             );

//             if (!downloadedFilename && retryCount < maxRetries) {
//               console.log(
//                 `🔄 Retrying download (attempt ${retryCount + 2})...`
//               );
//               // Wait before retry
...
//           // Save to Gallery table even if image download failed, but with null URL
...
//             productId: productId,
//             downloadStatus: downloadedFilename ? "success" : "failed",
//           });
...
//               `✅ Gallery image ${
//                 index + 1
//               } saved to Gallery table: ${downloadedFilename}`
//             );
//           } else {
//             console.log(
//               `⚠️ Gallery image ${
//                 index + 1
//               } record created but image download failed`
//             );
//           }
//         } else {
//           console.log(
//             `❌ All download attempts failed for gallery image ${index + 1}`
//           );
//         }
//       } catch (imgError) {
//         console.error(
//           `❌ Error processing gallery image ${index + 1}:`,
//           imgError.message
//         );
//       }
//     }
...
//     console.log(
//       `✅ Successfully processed ${successfulDownloads}/${galleryData.length} gallery images for product ID: ${productId}`
//     );
//   } catch (error) {
...
// // Add this improved main image download logic to your import functions
...
//         console.log(
//           `🔄 Attempting main image download (${i + 1}/${
//             imageUrls.length
//           }): ${imageUrl}`
//         );
...
//         if (downloadedFilename) {
//           console.log(
//             `✅ Main image downloaded successfully from attempt ${
//               i + 1
//             }: ${downloadedFilename}`
//           );
...
//       } catch (error) {
//         console.log(
//           `❌ Error in main image download attempt ${i + 1}:`,
//           error.message
//         );
//       }
...
//     console.log(
//       `❌ All main image download attempts failed for ${productCode}`
//     );
//     return null;
//   } catch (error) {
//     console.error(
//       `❌ Error in main image download with fallback:`,
//       error.message
//     );
//     return null;
//   }
...
// // 📌 FIXED Helper function to process a single product - COMPLETE VERSION
...
//       if (importProduct) {
//         // UPDATE: Always change status to "pending" for not found products
...
//         importStatus: "pending", // Ensure this is set to pending
//       };
//     }

//     if (response.status === 403 || response.status >= 500) {
//       if (importProduct) {
//         // UPDATE: Change status to pending for API errors
//         await importProduct.update({
//           status: "pending",
//           lastUpdated: new Date(),
...
//       if (importProduct) {
//         // UPDATE: Change status to pending for invalid responses
//         await importProduct.update({
//           status: "pending",
//           lastUpdated: new Date(),
...
//       );
//       if (importProduct) {
//         // UPDATE: Change status to pending for Icecat errors
//         await importProduct.update({
//           status: "pending",
//           lastUpdated: new Date(),
...
//     if (importProduct) {
//       // UPDATE: Always change status to pending for any error
//       await importProduct.update({
//         status: "pending",
//         lastUpdated: new Date(),
...
// // Update the importFromProductForImport function in productController.js
...
//       distributor,
//       autoCleanup = false,
//     } = req.body; // Changed autoCleanup default to false
...
//         // UPDATE: Change status to pending instead of keeping as inactive
//         await importProduct.update({
//           status: "pending",
//           lastUpdated: new Date(),
//           errorMessage: error.message,
...
//           importStatus: "pending", // Ensure status is set to pending
//         });
//       }
//     }
...
//     // UPDATE: Only run auto-cleanup if explicitly requested and brand is provided
...
//           currentStatus: f.importStatus || "pending", // Show updated status
//         })),
...
// // Update the autoCleanupFailedProducts function in productController.js
...
//     // UPDATE: Only proceed if there are actual failed items and brand is provided
...
//     // UPDATE: Only delete if there are products to delete
...
//       // UPDATE: Only delete from product_for_import if explicitly requested
//       // We're changing this to NOT delete from product_for_import
...
//     if (!sku || !brand)
//       return res.status(400).json({ error: "SKU and brand are required" });
...
//       return res
//         .status(404)
//         .json({ error: `Product not found in import list: ${sku} - ${brand}` });
...
//     res
//       .status(500)
//       .json({ error: error.message || "Failed to activate product" });
//   }
// };
...
//     res
//       .status(500)
//       .json({ error: error.message || "Failed to fetch products for import" });
//   }
// };
...
// // Add connection pool configuration OUTSIDE the function
...
// // CORRECTED downloadImage function
// const downloadImage = async (url, filename) => {
//   try {
//     console.log(`🖼️ Attempting to download image from: ${url}`);
...
// // IMPROVED: Process gallery images with concurrency control
// const processGalleryImages = async (galleryData, productId, productCode) => {
//   try {
//     if (!galleryData || !Array.isArray(galleryData)) {
//       console.log("❌ No gallery data found or invalid format");
...
// // Helper function to ensure subcategory exists
// const ensureSubCategoryExists = async (categoryName, categoryId) => {
//   try {
//     if (!categoryName) {
//       let defaultSubCategory = await SubCategory.findOne({
...
//   } catch (error) {
//     console.error("Error ensuring subcategory exists:", error);
//     const fallbackSubCategory = await SubCategory.findOne({ 
//       where: { title: "Uncategorized" } 
//     }) || await SubCategory.create({
//       title: "Uncategorized",
//       parentId: categoryId,
...
// // Helper function to process a single product - COMPLETE VERSION
// const processSingleProduct = async (
//   productData,
//   jobId = null,
//   importProduct = null
